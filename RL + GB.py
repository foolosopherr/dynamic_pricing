...
Приводим даты, режем неделю на 3 корзины: Пн–Чт, Пт, Сб–Вс.


Парсим промо-интервалы, собираем дневные данные в агрегат по (STORE×SKU×неделя×корзина).


Считаем признаки: лаги/скользящие средние по спросу/цене/доступности, простые иерархические поля (FAMILY/CATEGORY/SEGMENT), прокси-сток AVAIL_STOCK.


Для каждой корзины вычисляем BUCKET_START — точку «отрезания будущего».




«Снимок корзины» на момент запуска




Для цели (например, Пт) создаём снимок всех (STORE×SKU), которые встречались до старта этой корзины.


Если в данных нет явных строк для нужной корзины, переносим последние известные значения (цены, запасы, лаги/MA, промо-флаг) — чтобы всегда было что оптимизировать.




Оракул спроса (CatBoost)




Обучаем CatBoost на всей истории строго до старта текущей корзины (включая более ранние корзины той же недели).


Модель предсказывает спрос под заданный контекст и любую кандидатную цену.




Контекстный бандит (ценовой выбор)




Формируем дискретную лестницу цен (мультипликаторы, напр. 0.90…1.30) с бизнес-ограничениями: мин-маржа, лимит скачка vs последняя цена, округления.


Для каждого кандидата прогнозируем спрос → обрезаем по доступному стоку (OOS-guard) → считаем прибыль.


Добавляем UCB-бонус (лёгкая разведка) и штраф за повтор того же ARM (чтобы не «залипать»).


Выбираем ARM с максимумом скоринга → рекомендуем цену на всю корзину.




Роллинг-оценка (12 недель)




Для последних 12 появлений этой корзины повторяем процедуру «тренировка на прошлом → рекомендация на текущую корзину» и сравниваем с бэйзлайном (последняя/базовая цена).


Считаем KPI: прибыль, uplift, %-улучшение, стабильность действий.




Расписание запусков (офлайн, 3 раза в неделю)




Пн: считаем и фиксируем цены на Пн–Чт (используя всё прошлое до понедельника).


Пт: считаем Пт, уже видя факты Пн–Чт этой недели.


Сб: считаем Сб–Вс, уже видя факты пятницы.


Каждый запуск хранит выбранные цены и затем обновляет факты для следующего запуска.

Что это даёт

Решения причинно корректны (нет утечки будущего).

OOS предотвращается за счёт ограничения спроса доступным стоком и штрафов.

Цены не «залипают» на одном ARM, но и не прыгают — действуют гарды.

Иерархия и лаги/MA смягчают разреженность и холодный старт.

Есть понятная офлайн-оценка на скользящем окне 12 недель.
